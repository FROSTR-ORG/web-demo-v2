...
describe("CollectSharesScreen", () => {
  ...
  function renderCollect(routeState?: unknown) {
    return render(
      <MemoryRouter
        initialEntries={[
          { pathname: "/recover/test-profile-id", state: routeState },
        ]}
      >
        <Routes>
  ...
  it("renders real recovery inputs for the saved profile and required bfshare slot", () => {
    ...
    expect(
      screen.getByLabelText("Source Share #2 bfshare package"),
    ).toBeInTheDocument();
    expect(
  ...
  it("validates sources before enabling real recovery", async () => {
    renderCollect();
    expect(
      screen.getByRole("button", { name: "Validate Sources" }),
    ).toBeDisabled();
    ...
    await waitFor(() =>
      expect(mockApp.state.validateRecoverSources).toHaveBeenCalledWith({
    ...
    expect(mockNavigate).toHaveBeenCalledWith(
      "/recover/test-profile-id/success",
    );
  });
  ...
  it("renders real AppState validation errors", async () => {
    mockApp.state.validateRecoverSources = vi.fn(async () => {
      throw new Error(
        "The profile password could not decrypt Source Share #1.",
      );
    });
    renderCollect();
    fireEvent.change(screen.getByLabelText("Saved profile password"), {
...
describe("RecoverSuccessScreen", () => {
  ...
  function renderSuccess(routeState?: unknown) {
    return render(
      <MemoryRouter
        initialEntries={[
          { pathname: "/recover/test-profile-id/success", state: routeState },
        ]}
      >
        <Routes>
          <Route
            path="/recover/:profileId/success"
            element={<RecoverSuccessScreen />}
          />
        </Routes>
      </MemoryRouter>,
    );
  ...
  it("copies, reveals, and clears the real recovered nsec", async () => {
    ...
    await waitFor(() =>
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith(fullNsec),
    );
