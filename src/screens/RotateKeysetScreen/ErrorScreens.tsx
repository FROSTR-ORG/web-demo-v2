...
export function RotateWrongPasswordScreen() {
  ...
  const state = location.state as { profileId?: string; errorMessage?: string; details?: { sourceIndex?: number; source?: string } } | null;
  const routeState = state?.profileId ? { profileId: state.profileId } : undefined;
  const sourceLabel = state?.details?.source === "saved_profile" ? "Source Share #1" : `Source Share #${state?.details?.sourceIndex ?? 2}`;
  const message = state?.errorMessage ?? "Wrong password. Unable to decrypt this bfshare source package.";
  ...
  return (
    <AppShell headerMeta={MOCK_SOURCE_SHARE_1.label} mainVariant="flow">
      <div className="screen-column">
        ...
        <Button type="button" size="full" onClick={() => navigateWithRotateState(navigate, "/rotate-keyset", routeState)}>
          Retry
        </Button>
      </div>
    </AppShell>
...
export function RotateGroupMismatchScreen() {
  const navigate = useNavigate();
  const location = useLocation();
  const state = location.state as { profileId?: string; errorMessage?: string; details?: { groupPublicKey?: string; shareIndex?: number; sourceIndex?: number } } | null;
  const routeState = state?.profileId ? { profileId: state.profileId } : undefined;
        ...
        <div className="rotate-error-card mismatch">
          ...
          <div className="rotate-mismatch-keys">
            <div className="rotate-mismatch-row">
              ...
              <span className="rotate-mismatch-value blue">{state?.details?.groupPublicKey ? `${state.details.groupPublicKey.slice(0, 10)}...${state.details.groupPublicKey.slice(-4)}` : "npub1qe3...7kkm"}</span>
            </div>
            <div className="rotate-mismatch-row last">
              <span className="rotate-mismatch-label">Share #{state?.details?.sourceIndex ?? 2}{state?.details?.shareIndex !== undefined ? "" : " Group"}</span>
              <span className="rotate-mismatch-value red">{state?.details?.shareIndex !== undefined ? `Index ${state.details.shareIndex}` : "npub1x7f...2mnp"}</span>
            </div>
          </div>

          <p className="rotate-mismatch-help">
            {state?.errorMessage ?? "All bfshare source packages must match the same current group configuration and group public key. Replace one source and retry."}
          </p>
        </div>
  ...
  return (
    <AppShell headerMeta={MOCK_SOURCE_SHARE_1.label} mainVariant="flow">
      <div className="screen-column">
        ...
        <Button type="button" size="full" onClick={() => navigateWithRotateState(navigate, "/rotate-keyset/review", routeState)}>
          Retry Generation
        </Button>
      </div>
    </AppShell>
