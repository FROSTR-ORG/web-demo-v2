...
export function RotateGenerationProgressScreen() {
  ...
  const blocked =
    !rotatePhaseAtLeast(rotateKeysetSession, "rotated") && !demoProgress;
  const [phases, setPhases] = useState<RotatePhase[]>(() =>
    seedRotatePhases(
      ROTATE_INITIAL_PHASES,
      demoUi.progress?.completedCount,
      demoUi.progress?.activeIndex,
    ),
  );
        ...
        <div className="generation-progress-card">
          {phases.map((phase) => (
            ...
            >
              ...
              {phase.state === "done" && (
                <span className="generation-phase-status">Done</span>
              )}
              {phase.state === "active" && (
                <span className="generation-phase-status">Processing...</span>
              )}
            </div>
          ))}
        </div>
    ...
    return (
      <span className="generation-phase-dot done">
        ...
        >
          ...
          <path
            d="M6.75 10.25L8.9 12.35L13.25 7.95"
            stroke="#4ADE80"
            strokeWidth="1.5"
            strokeLinecap="round"
