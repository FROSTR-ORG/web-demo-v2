...
export const demoScenarios: DemoScenario[] = [
  ...
  scenario("dashboard-loading-profile", "dashboard", "Dashboard - 1b. Loading Profile", "screens/dashboard/1b-loading-profile", `/dashboard/${DEMO_PROFILE_ID}/loading`, dashboardState, "Loading profile..."),
  scenario("dashboard-profile-load-failed", "dashboard", "Dashboard - 1b Error. Profile Load Failed", "screens/dashboard/1b-error-profile-load-failed", `/dashboard/${DEMO_PROFILE_ID}/profile-load-failed`, dashboardState, "Couldn’t load profile"),
  ...
  variantScenario("dashboard-peer-policy-chips", "dashboard-policies", "dashboard", "Dashboard - 1c. Policies (runtime chips)", "screens/dashboard/1c-policies", `/dashboard/${DEMO_PROFILE_ID}`, dashboardState, "Peer Policies", { demoUi: { dashboard: { showPolicies: true, hideMockControls: true, paperPanels: false } } }),
  scenario("dashboard-stopped", "dashboard", "Dashboard - 2. Stopped", "screens/dashboard/2-stopped", `/dashboard/${DEMO_PROFILE_ID}`, dashboardState, "Signer Stopped", { demoUi: { dashboard: { state: "stopped", hideMockControls: true, paperPanels: true } } }),
  scenario("dashboard-relays-offline", "dashboard", "Dashboard - 2b. All Relays Offline", "screens/dashboard/2b-all-relays-offline", `/dashboard/${DEMO_PROFILE_ID}`, dashboardState, "All Relays Offline", { demoUi: { dashboard: { state: "relays-offline", hideMockControls: true, paperPanels: true } } }),
  scenario("dashboard-signing-blocked", "dashboard", "Dashboard - 2c. Signing Blocked", "screens/dashboard/2c-signing-blocked", `/dashboard/${DEMO_PROFILE_ID}`, dashboardState, "Signing Blocked", { demoUi: { dashboard: { state: "signing-blocked", hideMockControls: true, paperPanels: true } } }),
  ...
  // Adapted from Shared flow Paper sources; reference PNGs reused from the Shared flow until dedicated captures exist.
  {
    ...scenario(
      ...
      dashboardState,
      "Create Profile",
      { demoUi: { rotateKeyset: { passwordPreset: DEMO_PASSWORD }, shared: { profileNamePreset: "Igloo Web", passwordPreset: DEMO_PASSWORD, relayPreset: "wss://relay.example.com" } } }
    ),
  ...
  },
  {
    ...scenario(
      ...
      dashboardState,
      "Distribute Shares",
      { demoUi: { rotateKeyset: { passwordPreset: DEMO_PASSWORD }, shared: { lockedPackageIndexes: [2] } } }
    ),
  ...
  scenario("recover-success", "recover", "Recover - 1b. Recover Success", "screens/recover/1b-recover-success", `/recover/${DEMO_PROFILE_ID}/success`, dashboardState, "Security Warning", { demoUi: { recover: { copied: true } } })
];
