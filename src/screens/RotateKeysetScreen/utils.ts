...
export function rotatePhaseAtLeast(session: { phase?: string } | null | undefined, phase: RotateProductPhase): boolean {
  const current = session?.phase;
  return Boolean(current && current in ROTATE_PHASE_ORDER && ROTATE_PHASE_ORDER[current as RotateProductPhase] >= ROTATE_PHASE_ORDER[phase]);
}
